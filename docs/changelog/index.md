...
- 代理规则支持独立配置连接、发送、读取与客户端超时、请求体大小、WebSocket 及请求/响应缓冲行为；网站域名表单仅维护域名与凭证，避免重复配置代理参数。
- 代理规则超时改用带单位的 Nginx duration（如 `30s`、`2m`），并兼容读取旧版数值秒数；升级时会迁移无歧义的旧域名级 Nginx 设置，多域名绑定等无法安全判断的配置需在规则管理中重新确认。
- 控制台新增繁體中文界面，并同步更新代理规则 API 的 Swagger 契约。
...
- 全球態勢板固定以綠、黃、紅狀態點呈現節點，同區域節點會聚合顯示數量與節點清單，並依節點數導向詳情或列表。
- 反向代理路由支援設定主用與備用兩個優先層級的多個上游，可依相同優先級使用輪詢或最少連接負載均衡，備用上游會在主用上游不可用時接替流量。
- Cloudflare 指向分組支援多主多備節點優先級，會為目前最低可用優先級的全部在線節點維護 A 記錄，並清理本系統不再使用的舊記錄。
...
- 重构 Go 包依赖结构为统一模块（Monorepo），模块命名为 `github.com/XiaoLong-Taiwan/openflare-longshang`
