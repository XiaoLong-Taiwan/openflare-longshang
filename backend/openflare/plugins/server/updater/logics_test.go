...
func TestParseRepository(t *testing.T) {
	tests := []struct {
		...
		{name: "short form", input: "XiaoLong-Taiwan/openflare-longshang", want: "XiaoLong-Taiwan/openflare-longshang"},
		{name: "GitHub URL", input: "https://github.com/XiaoLong-Taiwan/openflare-longshang.git", want: "XiaoLong-Taiwan/openflare-longshang"},
		{name: "unsupported host", input: "https://example.com/XiaoLong-Taiwan/openflare-longshang", wantErr: true},
...
func TestSelectLatestRelease(t *testing.T) {
	...
	release, asset, err := selectLatestRelease("XiaoLong-Taiwan/openflare-longshang", releases)
	if err != nil {
...
func TestSelectLatestReleaseWithCustomRepo(t *testing.T) {
	...
	release, asset, err := selectLatestRelease("xiaolong-taiwan/PixezSync", releases)
	if err != nil {
		t.Fatalf("selectLatestRelease() error = %v", err)
	}
	if release.TagName != "v2.0.0" {
